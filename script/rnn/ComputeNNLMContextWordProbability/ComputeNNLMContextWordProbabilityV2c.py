...
Still need to load the entire output cache or multiple times.
"""
...
def renormalize_unigrams(data_sequence, outputs_cache, id_to_word, temperatures, max_frequency=5,
	...
	for word_id in id_to_word:
		log_s_table[word_id] = numpy.zeros(1 + max_frequency) - 1e3
		log_s_table[word_id][1] = 0
	...
	for i in range(len(data_sequence)):
		...
		log_s_table[word_id][1:] = numpy.logaddexp(
			log_s_table[word_id][1:] + numpy.log(1 - numpy.exp(sample_log_p_word_context) + 1e-99),
			log_s_table[word_id][:max_frequency] + sample_log_p_word_context)
		...
		if len(context_window) == context_window_size:
			...
			for word_id in word_candidates:
				...
				if word_id not in log_s_table[context_ids]:
					log_s_table[context_ids][word_id] = numpy.zeros(1 + max_frequency) - 1e3
					log_s_table[context_ids][word_id][1] = 0

				assert 0 < 1 - numpy.exp(sample_log_p_word_context) <= 1, numpy.exp(sample_log_p_word_context)

				# if context_ids[0] == 61:
				# print([id_to_word[context_id] for context_id in context_ids], id_to_word[word_id], word_id, sample_log_p_word_context)

				log_s_table[context_ids][word_id][1:] = numpy.logaddexp(
					log_s_table[context_ids][word_id][1:] + numpy.log(1 - numpy.exp(sample_log_p_word_context) + 1e-99),
					log_s_table[context_ids][word_id][:max_frequency] + sample_log_p_word_context)
...
def main():
	...
	for context_window_size in range(0, 9):
		...
		if checkpoint == 1:
			for context_ids in log_s_table_context_word:
				for word_id in log_s_table_context_word[context_ids]:
					print(context_ids, word_id, count_context_word[context_ids][word_id],
					      log_E_c_context_word[context_ids][word_id], log_s_table_context_word[context_ids][word_id])
		'''
		...
		if (context_window_size > 0) or backoff_to_uniform:
			...
			for context_ids in log_s_table_context_word:
				for word_id in log_s_table_context_word[context_ids]:
					E_n_1 += numpy.exp(log_s_table_context_word[context_ids][word_id][2])
					E_n_2 += numpy.exp(log_s_table_context_word[context_ids][word_id][3])

			log_discounts[context_window_size] = numpy.log(E_n_1) - numpy.log(E_n_1 + 2 * E_n_2)
			# print(context_window_size, E_n_1, E_n_2, log_discounts[context_window_size])
			...
			for word_id in log_s_table_context_word[context_ids]:
				...
				if word_id in log_s_table_context_word[context_ids]:
					log_p_c_gt0_context_word[context_ids][word_id] = max(
						numpy.log(1 - numpy.exp(log_s_table_context_word[context_ids][word_id][1]) + 1e-99),
						scipy.misc.logsumexp(log_s_table_context_word[context_ids][word_id][2:]))
				...
				log_p_c_eq0_context_word[context_ids][word_id] = log_s_table_context_word[context_ids][word_id][1]
				# print(context_ids, word_id, log_p_c_eq0_context_word[context_ids][word_id] )
		...
		if context_window_size == 0 and (not backoff_to_uniform):
			...
			for context_ids in log_p_c_gt0_context_word:
				...
				#print("checkpoint", current._word_to_logprob[0], root._word_to_logprob[0])
		else:
			# log_E_c_context = {}
			for context_ids in log_s_table_context_word:
				current = find(root, context_ids)
				...
				for word_id in id_to_word:
					if word_id in log_s_table_context_word[context_ids]:
						...
						'''
						if checkpoint == 4:
							print(context_ids, word_id, log_E_c_context_word[context_ids][word_id],
							      log_discounts[context_window_size], log_p_c_gt0_context_word[context_ids][word_id],
							      current._log_E_n_ge1_context, log_p_word_given_temp_context)
						'''
						...
						for i in range(0, len(context_ids)):
							...
							if context_ids == (3, 4):
								if word_id in temp_node._word_to_logprob:
									print("found", word_id, temp, context_ids[i + 1:], temp_node._word_to_logprob[word_id])
								else:
									print("not found", word_id, temp, context_ids[i + 1:],
									      log_discounts[context_window_size - i - 1],
									      temp_node._log_E_n_ge1_context, temp_node._log_E_c_context)
							'''
					...
					#if context_ids==(3, 4):
						#print(word_id, log_E_c_context, temp)
				...
				#if checkpoint == 4:
					#print(context_ids, current._log_E_c_context)
