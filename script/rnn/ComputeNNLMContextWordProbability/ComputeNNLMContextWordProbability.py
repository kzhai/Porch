...
def unigram_to_trie(data_sequence, outputs_cache, root, id_to_word, temperatures, max_frequency=5,
	...
	for i in range(len(data_sequence)):
		...
		log_E_counts[word_id] = numpy.logaddexp(log_E_counts[word_id],
...
def main():
	...
	'''
	unigram_to_trie_time = timeit.default_timer()
	...
	unigram_to_trie_time = timeit.default_timer() - unigram_to_trie_time
	print("unigram to trie took %.2fs..." % (unigram_to_trie_time))
	sys.stdout.flush()
	if memory_footprint:
