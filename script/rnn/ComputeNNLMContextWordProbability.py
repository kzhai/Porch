...
def main():
	...
	for context_window in range(1, 9):
		...
		context_candidates = {}
		context = [id_to_word[id] for id in data_sequence[:context_window]]
		for i in range(context_window, len(data_sequence)):
			temp_context = " ".join(context)
			if temp_context not in context_candidates:
				context_candidates[temp_context] = []
			context_candidates[temp_context].append(id_to_word[data_sequence[i]])
		...
		for i in range(context_window + 1, len(data_sequence)):
			temp_context = " ".join(context)
			if temp_context not in context_word_probability:
				context_word_probability[temp_context] = []
			assert id_to_word[data_sequence[i]] in context_candidates[temp_context]
			candidate_probability = {word: outputs_cache[i - 1][word_to_id[word]] for word in
			                         context_candidates[temp_context]}
			context_word_probability[temp_context].append(
				(log_prob, id_to_word[data_sequence[i]], candidate_probability))
			...
			if len(context_word_probability) % 10000 == 0:
				print("processed %d %d-grams..." % (len(context_word_probability), context_window))

		print("processed %d %d-grams..." % (len(context_word_probability), context_window))

		context_word_probability_pkl_file = os.path.join(settings.output_directory, "context=%d.pkl" % context_window)
		pickle.dump(context_word_probability, open(context_word_probability_pkl_file, "wb"),
		            protocol=pickle.HIGHEST_PROTOCOL)

		context_word_probability_txt_file = os.path.join(settings.output_directory, "context=%d.txt" % context_window)
		context_probability_txt_stream = open(context_word_probability_txt_file, 'w')
		for context in context_word_probability:
			# context_probability_txt_stream.write("%s\n" % (context))
			for log_prob, word, candidate_probability in context_word_probability[context]:
				#print(candidate_probability)
				context_probability_txt_stream.write("%s\t%g\t%s\t%s\n" % (context, log_prob, word, " ".join(
					["%s:%g" % (word, candidate_probability[word]) for word in candidate_probability])))
